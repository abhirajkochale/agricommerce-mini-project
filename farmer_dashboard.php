...
<div class="container">
    <div class="flex-between mb-2">
        ...
        <div class="font-w-600 text-primary-dark">Welcome, <?php echo htmlspecialchars($_SESSION['user_name']); ?>!
        </div>
    </div>
                ...
                <table>
                    ...
                    <tbody>
                        ...
                        if (mysqli_num_rows($orders_result) > 0):
                            while ($orow = mysqli_fetch_assoc($orders_result)):
                                ...
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        $bg = '#eff6ff';
                                        $fg = '#2563eb';
                                        if ($orow['status'] == 'Shipped') {
                                            $bg = '#fff7ed';
                                            $fg = '#c2410c';
                                        }
                                        if ($orow['status'] == 'Delivered') {
                                            $bg = '#f0fdf4';
                                            $fg = '#15803d';
                                        }
                                        if ($orow['status'] == 'Cancelled') {
                                            $bg = '#f1f5f9';
                                            $fg = '#475569';
                                        }
                                        ?>
                                        <span
                                            style="display:inline-block; padding: 4px 10px; border-radius: 12px; background: <?php echo $bg; ?>; color: <?php echo $fg; ?>; font-size: 0.75rem; font-weight: 700;">
                                            <?php echo htmlspecialchars($orow['status']); ?>
                                        </span>
                                    </td>
                                    <td>
                                        <?php if ($orow['status'] !== 'Cancelled'): ?>
                                            <select name="statuses[<?php echo htmlspecialchars($orow['item_id']); ?>]"
                                                class="form-control"
                                                style="padding: 0.2rem; min-width: 100px; height:auto; font-size: 0.9rem;">
                                                <option value="Pending" <?php if ($orow['status'] == 'Pending')
                                                    echo 'selected'; ?>>
                                                    Pending</option>
                                                <option value="Shipped" <?php if ($orow['status'] == 'Shipped')
                                                    echo 'selected'; ?>>
                                                    Shipped</option>
                                                <option value="Delivered" <?php if ($orow['status'] == 'Delivered')
                            ...
                            <tr>
                                <td colspan="7" class="text-center p-8">No customer orders yet. Keep listing fresh crops!
                                </td>
                            </tr>
                        <?php endif;
